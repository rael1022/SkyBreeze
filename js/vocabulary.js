(() => {
  const vocabularySets = [
    ['Airport & Essentials', 'Travel-ready places'],
    ['Health & Care', 'Useful daily stops'],
    ['Campus Stops', 'Learn and eat together'],
    ['City Errands', 'Everyday city places'],
    ['Leisure Route', 'Relax with friends'],
    ['Shopping Trip', 'Find what you need'],
  ];

  document.querySelectorAll('.vocab-card').forEach((card, index) => {
    const set = vocabularySets[index];
    if (!set) return;
    card.querySelector('.vocab-word').textContent = set[0];
